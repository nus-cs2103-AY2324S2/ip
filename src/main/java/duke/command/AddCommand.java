...
public class AddCommand implements Command {
    public String execute(String command, String[] info, ItemList itemList) throws CustomExceptions {
        if (info[0].equals("todo")) {
            ...
            if (info.length == 1) {
                throw new CustomExceptions.invalidTaskException("Please re-enter duke.item.Deadline with a valid name");
            } else {
                try {
            ...
            if (info.length == 1) {
                throw new CustomExceptions.invalidTaskException("Please re-enter duke.item.Event with a valid name");
            } else {
                try {
                    return itemList.addEvent(info);
                } catch (CustomExceptions.toBeforeFromException e) {
                    throw new CustomExceptions.toBeforeFromException("Please re-enter duke.item.Event /from BEFORE /to: " + command);
