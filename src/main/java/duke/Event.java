...
public class Event extends Task {
     ...
     * @param isDone The initial completion status of the event task.
     *      True indicates that the event has been completed, while false indicates that it is still upcoming.
     ...
     * @throws DateTimeParseException If either the provided 'from' or 'to' string does not conform
     *      to the expected date and time format, indicating an issue with parsing the event's start or end time.
     */
     ...
     * The format includes the task type identifier ('E'),
     *      the completion status, the task description, and the event's time range,
    ...
    public String toFileFormat() {
        return TASK_TYPE
                + " | " + this.isDone + " | "
                + this.description + " | "
                + this.from.format(DateTimeFormatter.ofPattern(DATE_TIME_FORMAT_INPUT))
                + "-" + this.to.format(DateTimeFormatter.ofPattern(DATE_TIME_FORMAT_INPUT));
    }
    ...
    public String toString() {
        return "[" + TASK_TYPE + "]"
                + super.toString()
                + " (from: " + from.format(DateTimeFormatter.ofPattern(DATE_TIME_FORMAT_OUTPUT))
                + " to: " + to.format(DateTimeFormatter.ofPattern(DATE_TIME_FORMAT_OUTPUT)) + ")";
    }
}
