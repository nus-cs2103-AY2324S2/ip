package pingmebot;

import javafx.fxml.FXML;
import javafx.scene.control.Button;
import javafx.scene.control.ScrollPane;
import javafx.scene.control.TextField;
import javafx.scene.image.Image;
import javafx.scene.layout.AnchorPane;
import javafx.scene.layout.VBox;
/**
 * Controller for MainWindow. Provides the layout for the other controls.
 */
public class MainWindow extends AnchorPane {
    @FXML
    private ScrollPane scrollPane;
    @FXML
    private VBox dialogContainer;
    @FXML
    private TextField userInput;
    @FXML
    private Button sendButton;

    private PingMe pingMe;

    private Image userImage = new Image(this.getClass().getResourceAsStream("/images/UserImage.png"));
    private Image pingMeBotImage = new Image(this.getClass().getResourceAsStream("/images/PingMeBotImage.png"));

    /**
     * Helps to generate a dialog box to welcome the user when the user boots the application.
     */
    @FXML
    public void initialize() {
        scrollPane.vvalueProperty().bind(dialogContainer.heightProperty());
        String greetingMsg = "Hello! I'm PingMeBot\n" + "What can I do for you?";
        dialogContainer.getChildren().addAll(
                DialogBox.getPingMeBotDialog(greetingMsg, pingMeBotImage)
        );
    }

    public void setPingMe(PingMe pm) {
        pingMe = pm;
    }

    /**
     * Creates two dialog boxes, one echoing user input and the other containing Duke's reply and then appends them to
     * the dialog container. Clears the user input after processing.
     */
    @FXML
    private void handleUserInput() {
        String input = userInput.getText();
        String response = pingMe.getResponse(input);
        dialogContainer.getChildren().addAll(
                DialogBox.getUserDialog(input, userImage),
                DialogBox.getPingMeBotDialog(response, pingMeBotImage)
        );
        userInput.clear();
    }
}
