/**
 * The Message class provides a centralized repository of  strings used throughout 
 * the application to display various forms of messages.
 */
package util;

public class Messages {
    public static final String MESSAGE_WRONG_PARAMETERS = "Incorrect parameters";
    public static final String MESSAGE_NO_SUCH_ELEMENTS = "Index number does not exists, please use an index that exists in list.";
    public static final String MESSAGE_EMPTY_LIST = "There are no elements in the list, try adding some!";
    public static final String MESSAGE_SAVE_CORRUPTED = "Save file has been corrupted, starting anew.";
    public static final String MESSAGE_SAVE_FOUND = "Save file found and loaded.";
    public static final String MESSAGE_END_LINE = "______________________________________";
    
    public static final String MESSAGE_START_BOT =  "Hellooo! I'm \n" 
    + "  ____        _     ____        _   \n"
    + " |  _ \\      | |   |  _ \\      | |  \n"
    + " | |_) | ___ | |__ | |_) | ___ | |_ \n"
    + " |  _ < / _ \\| '_ \\|  _ < / _ \\| __|\n"
    + " | |_) | (_) | |_) | |_) | (_) | |_ \n"
    + " |____/ \\___/|_.__/|____/ \\___/ \\__|\n" 
    +"As I am a Chatbot, I therefore have no arms.\n" + 
    "Knock knock, who's there? Definitely not BobBot!" 
    + "Jokes aside, what can I do for you?";
}
